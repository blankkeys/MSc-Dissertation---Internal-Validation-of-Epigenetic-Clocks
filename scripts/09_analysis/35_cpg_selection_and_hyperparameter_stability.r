...
# Read a CSV if it exists and return NULL if it has not been created yet
read_existing_csv <- function(file) {
  if (!file.exists(file)) {
...
# Combine all available selected-CpG tables
selected_cpgs <- do.call(
  rbind,
  lapply(selected_cpg_files, read_existing_csv)
)

if (is.null(selected_cpgs) || nrow(selected_cpgs) == 0) {
  stop("No selected CpG files were available")
}

# Remove the intercept because it is the model baseline, not a CpG site
...
# Summarise how stable CpG selection is within one validation method
...
# One row per validation method with overall CpG stability statistics
...
# One row per CpG per validation method
...
# Summarise whether each validation method chooses similar alpha/lambda values across models
summarise_alpha_lambda <- function(validation_method, file) {
  performance_data <- read_existing_csv(file)
...
# Combine alpha/lambda stability summaries across methods
...
if (is.null(alpha_lambda_stability)) {
  alpha_lambda_stability <- data.frame()
}

# Save method-level stability, CpG-level frequency, CpG-set overlap and hyperparameter stability
write.csv(
