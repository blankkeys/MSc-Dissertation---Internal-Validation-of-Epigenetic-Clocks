...
select_method_hyperparameters <- function(source) {
  performance_data <- read.csv(source$performance_file, stringsAsFactors = FALSE)
  summary_data <- read.csv(source$summary_file, stringsAsFactors = FALSE)

  if (!"selected_alpha" %in% names(performance_data)) {
    stop("No selected_alpha column in ", source$performance_file)
  }
...
hyperparameter_selection <- do.call(
  rbind,
  lapply(seq_len(nrow(method_sources)), function(i) {
    select_method_hyperparameters(method_sources[i, ])
  })
)
