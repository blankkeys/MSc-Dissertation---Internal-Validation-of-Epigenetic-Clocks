# check downloaded IDAT files for GSE51032 external validation

# GSE51032 uses the 450K platform and provides raw IDAT files
# This check confirms red and green IDAT files are available as matched pairs

raw_tar_file <- "data/GSE51032/GSE51032_RAW.tar"
...
if (!file.exists(raw_tar_file)) {
  stop("The GSE51032 raw IDAT archive was not found")
}
...
if (length(green_idats) == 0 || length(red_idats) == 0) {
  stop("No GSE51032 red/green IDAT files were found")
}
...
if (length(missing_red) > 0 || length(missing_green) > 0) {
  stop("Some GSE51032 IDAT files do not have matching red/green pairs")
}
