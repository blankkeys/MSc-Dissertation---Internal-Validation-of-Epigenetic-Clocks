# remove failed samples from sample qc summary for GSE42861

library(minfi)

# Load the raw RGChannelSet and detection p-values created by previous steps
rgSet <- readRDS("data/GSE42861/rgset_raw.rds")
detp <- readRDS("data/GSE42861/sample_detection_pvalues.rds")
sample_qc <- read.csv("results/external_validation/qc/gse42861_sample_qc_summary.csv")
...
if (length(samples_to_keep) == 0) {
  stop("No GSE42861 samples passed sample QC")
}
...
saveRDS(rgSet_qc_pass, "data/GSE42861/rgset_qc_pass.rds")
saveRDS(detp_qc_pass, "data/GSE42861/sample_detection_pvalues_qc_pass.rds")
