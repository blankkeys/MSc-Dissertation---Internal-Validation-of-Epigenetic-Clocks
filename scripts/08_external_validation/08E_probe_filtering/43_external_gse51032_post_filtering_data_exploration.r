# Post-filtering data exploration for GSE51032
...
with_sex_chromosomes <- readRDS("data/GSE51032/mset_normalised_filtered_annotation_crossreactive_cpg.rds")

# Load the CpG-filtered object after autosomal filtering
without_sex_chromosomes <- readRDS("data/GSE51032/mset_normalised_filtered_annotation_crossreactive_cpg_autosomal.rds")
...
pdf("results/external_validation/qc/gse51032_with_sex_chromosomes_density_plot.pdf")
densityPlot(
  beta_with_sex_chromosomes,
  sampGroups = NULL,
  main = "GSE51032 with sex chromosomes: beta value density"
)
dev.off()

# Plot MDS before sex chromosome probe removal
pdf("results/external_validation/qc/gse51032_with_sex_chromosomes_mds_plot.pdf")
mdsPlot(
  beta_with_sex_chromosomes,
  numPositions = 10000,
  sampGroups = NULL,
  main = "GSE51032 with sex chromosomes: MDS plot"
)
dev.off()

# Plot MDS after sex chromosome probe removal
pdf("results/external_validation/qc/gse51032_without_sex_chromosomes_mds_plot.pdf")
mdsPlot(
  beta_without_sex_chromosomes,
  numPositions = 10000,
  sampGroups = NULL,
  main = "GSE51032 without sex chromosomes: MDS plot"
)
dev.off()
...
write.csv(
  ...
  "results/external_validation/qc/gse51032_post_filtering_data_exploration_summary.csv",
  row.names = FALSE
)
