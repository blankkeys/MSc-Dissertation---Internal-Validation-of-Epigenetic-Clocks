# Test internal-validation residual thresholds in the external GSE42861 dataset
...
external_predictions_file <- "results/external_validation/gse42861_external_validation_predictions.csv"
...
if (!file.exists(external_predictions_file)) {
  stop("GSE42861 external validation prediction file was not found")
}
...
write.csv(
  all_classifications,
  "results/external_validation/gse42861_threshold_classifications.csv",
  row.names = FALSE
)

write.csv(
  coverage_summary,
  "results/external_validation/gse42861_internal_threshold_coverage_summary.csv",
  row.names = FALSE
)
